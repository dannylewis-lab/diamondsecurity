...
import {
  ...
  Briefcase, PieChart, Zap, ArrowRight, BarChart2
} from 'lucide-react'
...
export default function ServicesPage() {
  return (
    <>
      {/* Hero */}
      <section className="pt-28 pb-16 bg-white border-b border-gray-100 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl">
            ...
            <h1 className="font-display text-4xl sm:text-5xl lg:text-[3.6rem] text-[#0a0a0a] leading-[1.08] mb-6">
              Three Licensed<br />
            ...
            <p className="text-gray-500 text-lg leading-relaxed mb-10 max-w-xl">
              ...
              each regulated by the CMSA, each built around your financial success.
            </p>
                ...
                >
                  ...
                  <div className="relative z-10 p-8 md:p-12 grid grid-cols-1 md:grid-cols-5 gap-8 items-start">
                    <div className="md:col-span-2">
                      <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-5 shadow-sm" style={{ background: s.mid }}>
                        <Icon size={26} style={{ color: s.accent }} />
                      </div>
                      <div className="text-xs font-bold tracking-[0.2em] uppercase mb-2" style={{ color: s.accent }}>
                        Service {s.number}
                      </div>
                      <h3 className="text-2xl font-extrabold text-gray-900 leading-snug mb-2">{s.title}</h3>
                    ...
                    <div className="md:col-span-3">
                      <p className="text-gray-600 leading-relaxed text-[15px] mb-7">{s.desc}</p>
                      <div className="h-px mb-7 opacity-30" style={{ background: s.accent }} />
                      <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {s.features.map((f) => (
                          <li key={f} className="flex items-start gap-2.5 text-sm text-gray-700">
                            <CheckCircle size={15} className="shrink-0 mt-0.5" style={{ color: s.accent }} />
                            {f}
                          </li>
                        ))}
                      </ul>
                  ...
                  <div className="h-1 w-0 group-hover:w-full transition-all duration-700" style={{ background: `linear-gradient(90deg, ${s.accent}, transparent)` }} />
                </div>
              )
            })}
          </div>
